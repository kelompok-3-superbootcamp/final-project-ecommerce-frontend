...
export default function Review() {
  ...
  const handleSubmit = e => {
    ...
    axios
      .post(`${host}/reviews`, body, { headers: header })
      .then(res => {
        Swal.fire({
          icon: "success",
  ...
  return (
    <form onSubmit={handleSubmit} ref={formRef} className="m-auto flex max-w-xl flex-col gap-4 p-8">
      <h1 className="m-auto text-2xl font-bold">Selamat Mobil mu dah sampai, Review yuk!</h1>
      {car ? (
        <ListCar
          id={car.data.id}
      ...
      <input
        type="submit"
        value="Kirim Review mu sekarang"
        className="cursor-pointer rounded-lg bg-slate-600 p-2 font-bold text-white"
      />
    </form>
  )
}
